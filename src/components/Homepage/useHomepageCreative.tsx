...
import {CtaLinkCardProps} from '@/components/web/cards'
import {useModal} from '@/components/web/content'
...
export default function useHomepageCreative(): useHomepageCreativeReturn {
  ...
  const evergrowthModal = useModal({
    ...
    content: <>
      ...
      <List>
        ...
        <ListItem><ChevronRight /> MC&lsquo;d and led many of the sessions</ListItem>
      </List>
    </>,
    actions: <Button href={links.evergrowthMastermind} target="_blank">
      Have a Listen
    </Button>,
  })
  const evergrowth = {
    ...
    links: [
      ...
      {
        label: 'Have a Listen',
        href: links.evergrowthMastermind,
      },
    ],
  }
  ...
  const hooModal = useModal({
    ...
    actions: <Button href={links.homerOnOlympus} target="_blank">
      Have a Listen
    </Button>,
  })
